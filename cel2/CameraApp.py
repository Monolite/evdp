...
class CameraApp(GuiHandler):
    ...
    def __init__(self, device):
    	...
    	print self.texture.fromCamera()
         	
        scale = self.texture.getTexScale()
    	#self.texture = OpenCVTexture()
    	
        if not self.texture.getVideoWidth() or not self.texture.getVideoHeight():
                print "invalid camera texture size", self.texture.getVideoWidth(), self.texture.getVideoHeight()
        ...
        #self.card.setHpr(7.0419e-05, 35.5468, -7.62941e-06)
        self.card.setPos(0.002248, 1.07841, 0.413093)
        self.card.hide()
