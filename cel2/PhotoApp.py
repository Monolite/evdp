...
class PhotoApp(GuiHandler):    
    ...
    def activate(self, events):
        ...
        events.add_action("centro", self.change_mode)
        events.add_action("boton_izq", self.change_mode)
    ...
    def refresh_gui(self):
        ...
        if self.thumb_viewer:
            self.set_function_1_name("Expandir")
        else:
            self.set_function_1_name("Galeria")
                
        GuiHandler.refresh_gui(self)
        
        text = "Imagen " + str(self.selected_index + 1) + " de " + str(len(self.image_list))
        self.cr.set_font_size(18) 
        bounds = self.cr.text_extents(text)
        self.cr.move_to(self.width/2.0 - (bounds[2]/2.0), self.height - 65)
